...
| **Business Overview sales / purchases** | `business_overview_config.sales_store_id` / `purchases_store_id` (+ `shopify_store_ids`, `quotation_statuses`, `timezone`) | Business Overview (invoices, sales/margin trend, purchase orders; Shopify cost lookup falls back to Item Tracker S2S) |
...
- **Business Overview** (`/api/business-overview/*`) — owner dashboard: KPI strip, revenue/profit/margin trend (BackOffice `InvoicesDetails` revenue = `ExtendedPrice`, cost = `QtyShipped*UnitCost`; Shopify from the local mirror by order `created_at`, cost via `Items_tbl.UnitCost` by barcode), quotations in progress (DB_ADMIN, configurable statuses), open/shipped invoices (shipped = non-blank `TrackingNo`, dated by `COALESCE(ShipDate, InvoiceDate)`), purchase orders (incoming = `Status = 0 AND TotQtyRcv < TotQtyOrd`; purchased by `PoDate`; received by `PurchaseOrdersDetails.DateReceived`), drill-in modals for quotations/invoices/POs. Config singleton `business_overview_config`; BackOffice sales totals honour the Sales `sales_exclusions` list; DB_ADMIN via the soft admin-store resolver. Per-widget endpoints + one `/summary`; every block carries `configured`/`error` so one bad store never blanks the page. `/alerts` evaluates the operational alert rules (unshipped past cutoff, open-invoice age, stuck quotations, overdue POs, Shopify on-hold / unfulfilled aging / stale sync) against thresholds stored in `business_overview_config.alert_rules` (defaults + merge/validation in `schemas.py`: `BOV_DEFAULT_ALERT_RULES`, `bov_merge_alert_rules`, `bov_validate_alert_rules`); money rules (margin floor, revenue drop) are evaluated client-side from the summary. Previous-period math lives only in the backend (`period` is echoed to the client). Frontend: `bov*` section in `app.js`, hand-rolled SVG charts.
