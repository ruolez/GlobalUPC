...
class BOVInvoiceLine(BaseModel):
    ...
    item_unit_cost: Optional[float] = None    # this store's current Items_tbl.UnitCost (reference)
...
class BOVInvoiceDetailResponse(BaseModel):
    ...
    store_name: Optional[str] = None
    cost_basis: str = "sale"                      # sale = invoice line cost, current = store Items_tbl.UnitCost, s2s = S2S Items_tbl.UnitCost
...
class BOVSalesTrendResponse(BaseModel):
    ...
    cost_mode: str = "sale"                   # sale | current | s2s
    warnings: List[str] = []
...
class BOVShopifyOrderLine(BaseModel):
    ...
    unit_price: Optional[float] = None
    discounted_total: Optional[float] = None     # covers the ORIGINAL quantity
    line_revenue: Optional[float] = None         # discounted_total pro-rated by current_quantity / quantity
...
class BOVShopifyOrderHeader(BOVShopifyOrderRow):
    ...
    revenue: Optional[float] = None              # Σ line_revenue (refunded units excluded)
    cost: Optional[float] = None
...
class BOVProductsResponse(BOVBlockStatus):
    ...
    cost_store_id: Optional[int] = None
    cost_store_name: Optional[str] = None
    cost_mode: str = "sale"                   # basis of the local_* columns: sale | current
...
class BOVProductLineRow(BaseModel):
    ...
    item_unit_cost: Optional[float] = None    # BackOffice: current Items_tbl.UnitCost
...
class BOVProductLinesResponse(BOVBlockStatus):
    ...
    cost_store_name: Optional[str] = None
    warnings: List[str] = []
    cost_mode: str = "sale"                   # basis of the local_* fields: sale | current
...
class MonthEndRow(BaseModel):
    ...
    ship_state: Optional[str] = None             # BackOffice Invoices_tbl.ShipState | Shopify shipping address province code
    sales_rep_id: Optional[int] = None           # BackOffice Invoices_tbl.SalesRepID (0/NULL = no rep)
    sales_rep: Optional[str] = None              # BackOffice: Employees_tbl.FirstName joined on EmployeeID = SalesRepID
